...
public class ChainLightning extends Spell {
    ...
    public ChainLightning() {
        super( "Цепная молния", new Random().nextInt( 36 ) );
    }
    ...
    public void cast( Wizard wizard, List<Character> characterList ) {
        ...
        characterList.stream()
                .filter( character-> character != null && !character.equals( wizard ) )
        ...
        if( nameCharacter.size() > 0 ) {
            System.out.printf("Цепная молния ударяет по %s. Каждый получает %s урона.\n", nameCharacter, this.getRange());
        }
    }
}
